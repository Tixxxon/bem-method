...
Цель документа — показать, как разрабатывать динамические БЭМ-проекты. После прочтения, вы сможете создавать собственные проекты, ориентированные на динамические данные.
...
* [Social Services Search Robot](#Приложение-social-services-search-robot) —  приложение, которое позволяет искать последние твиты и видео по ключевому слову.
...
У разработчиков есть традиция: начинать программирование на новом языке или фреймворке с приложения Hello, World. Приложение обычно выводит слова «Hello, World!» в выходной поток, демонстрируя тем самым, что оно запускается и может выполнять операции ввода/вывода.
...
Для создания понадобится локальная копия [шаблонного репозитория bem-express](#Шаблонный-репозиторий).

### Шаблонный репозиторий

Для решения задач по разработке динамических приложений создан шаблонный репозиторий [bem-express](https://github.com/bem/bem-express). Он содержит необходимый минимум конфигурационных файлов и решает целый класс задач, таких как сборка проекта, настройка линтеров, подключение библиотек и др.
...
7. Откройте браузер и введите адрес [localhost:3000](http://localhost:3000).
    ...
    >
    > 1. Отключите опцию **Общественные сети**.
